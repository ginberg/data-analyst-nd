...
print("Acids are an important component of wine and it constists of a fixed and 
      volatile part. This feature represents the fixed part and can be tartaric 
      acid for example. The mean and median are both about 7 g/dm^3. There are 
      some outliers at the right side with a maximum of almost 16 g/dm^3.")

#volatile.acidity
ggplot(data = wine, aes(x=volatile.acidity)) + geom_histogram()
summary(wine$volatile.acidity)
print("Volatile acidity is the amount of acetic acid in wine, which at too high 
      of levels can lead to an unpleasant, vinegar taste. The mean and median 
      are both about 0.30 g/dm^3. There are some outliers at the right side with
      a maximum of almost 1.58 g/dm^3.")

#total.acidity
ggplot(data = wine, aes(x=total.acidity)) + geom_histogram()
summary(wine$total.acidity)
print("Total acidity is the sum of fixed and volatile acidity. As can be 
      expected from previous plots, there are some outliers at the right side.")

#citric.acid
ggplot(data = wine, aes(x=citric.acid)) + geom_histogram()
summary(wine$citric.acid)
print("Citric acid can add 'freshness' and flavor to wines. It has a 'normalish'
      distribution with a small peak at the left side and again some outliers at
      the right side, with a maximum value of 1.66 g/dm^3.")

#residual.sugar
ggplot(data = wine, aes(x=residual.sugar)) + geom_histogram()
summary(wine$residual.sugar)
print("Residual sugar is the amount of sugar remaining after fermentation stops.
      The distribution looks like the right side of a normal distribution, with
      a peak at 0 g/dm^3. The mean is quite bigger than the median, which is 
      caused by one or more big outlier(s).")

#chlorides
ggplot(data = wine, aes(x=chlorides)) + geom_histogram()
summary(wine$chlorides)
print("Chlorides is the amount of salt in the wine. It has a 'normalish' 
      distribution around +- 0.05 g/dm³ with some outliers at the right side.")

#free.sulfur.dioxide
ggplot(data = wine, aes(x=free.sulfur.dioxide)) + geom_histogram()
summary(wine$free.sulfur.dioxide)
print("Free sulfur dioxide is the free form of SO2. It has a 'normalish' 
      distribution around +- 30 mg/dm^3 with some very high 
      outliers (max=289 mg/dm^3)")

#total.sulfur.dioxide
ggplot(data = wine, aes(x=total.sulfur.dioxide)) + geom_histogram()
summary(wine$total.sulfur.dioxide)
print("The total sulfur dioxide is the amount of free and bound forms of S02. 
      It has a 'normalish' distribution with a mean of +-116 mg/dm^3. 
      There are some outliers with a high value.")

#density
ggplot(data = wine, aes(x=density)) + geom_histogram()
summary(wine$density)
print("The density represents the density of the wine, which is dependent on the
      percent alcohol and sugar content. It has a 'normalish' distribution with
    a mean of +- 0.995 and a max value of 1.0390 which is clearly an outlier.")

#pH
ggplot(data = wine, aes(x=pH)) + geom_histogram()
summary(wine$pH)
print("The pH features describes how acidic or basic a wine is on a scale from 0
      (very acidic) to 14 (very basic). It has a 'normalish' distribution with
      a mean of +- 3.2 and a max value of +- 4.0 which is an outlier.")

#sulphates
ggplot(data = wine, aes(x=sulphates)) + geom_histogram()
summary(wine$sulphates)
print("The sulphates is a wine additive wich acts as an antimicrobial and 
      antioxidant.It has a 'normalish' distribution with
      a mean of +- 0.53. It is clear from the plot that there are some outliers 
      at the right side (max = 2.0).")

#alcohol
ggplot(data = wine, aes(x=alcohol)) + geom_histogram()
summary(wine$alcohol)
print("The percent alcohol content of the wine. The mean and median are a little
      more than 10%, with some outliers at the high percentages and a maximum of
      14,9%.")

#color
ggplot(data = wine, aes(x=color)) + geom_histogram()
summary(wine$color)
print("The wine color which can either be red or wine. There are about 3 times
      as many white wines as red wines in this dataset.")

#quality
ggplot(data = wine, aes(x=quality)) + geom_bar(binwidth=1)
summary(wine$quality)
print("Quality is a score between 0 and 10. It has a 'normalish' distribution
      around 6 with a min and max of 3 and 9 respectively.")
```
...
#fixed.acidity
ggplot(data = wine, aes(y=fixed.acidity, x=factor(quality))) + 
...
#total.acidity
ggplot(data = wine, aes(y=total.acidity, x=factor(quality))) + 
  geom_jitter(alpha=0.4)+geom_boxplot(alpha=0.4)

#citric.acid
ggplot(data = wine, aes(y=citric.acid, x=factor(quality))) + 
  geom_jitter(alpha=0.4)+geom_boxplot(alpha=0.4)

#residual.sugar
ggplot(data = wine, aes(y=residual.sugar, x=factor(quality))) + 
  geom_jitter(alpha=0.4)+geom_boxplot(alpha=0.4)

#chlorides
ggplot(data = wine, aes(y=chlorides, x=factor(quality))) + 
  geom_jitter(alpha=0.4)+geom_boxplot(alpha=0.4)


#free.sulfur.dioxide
ggplot(data = wine, aes(y=free.sulfur.dioxide, x=factor(quality))) + 
  geom_jitter(alpha=0.4)+geom_boxplot(alpha=0.4)


#total.sulfur.dioxide
ggplot(data = wine, aes(y=total.sulfur.dioxide, x=factor(quality))) + 
  geom_jitter(alpha=0.4)+geom_boxplot(alpha=0.4)


#density
ggplot(data = wine, aes(y=density, x=factor(quality))) + 
  geom_jitter(alpha=0.4)+geom_boxplot(alpha=0.4)

#pH
ggplot(data = wine, aes(y=pH, x=factor(quality))) + 
  geom_jitter(alpha=0.4)+geom_boxplot(alpha=0.4)

#sulphates
ggplot(data = wine, aes(y=sulphates, x=factor(quality))) + 
  geom_jitter(alpha=0.4)+geom_boxplot(alpha=0.4)
...
#color
ggplot(data = wine, aes(y=color, x=factor(quality))) + 
  geom_jitter(alpha=0.4)+geom_boxplot(alpha=0.4)

#pH vs citric.acid
ggplot(data = wine, aes(x=citric.acid, y=pH)) + geom_point(alpha=0.2) + 
  xlim(0,0.75) + ylim(2.5, 3.75) + stat_smooth(method = "lm", col='red')
...
printRegression = function(relation){
  for (i in levels(wine$quality_level)) { 
    print(i)
    print(lm(data=wine[wine$quality_level %in% i,], relation)$coefficients[2])
  }  
}

#alcohol vs sulphates, color by quality_level
ggplot(data = wine, aes(x=sulphates, y=alcohol, color=quality_level)) + 
  geom_point() +
   geom_smooth(method="lm", fill=NA)
printRegression(alcohol ~ sulphates)
#TODO
...
ggplot(data = wine, aes(x=chlorides, y=alcohol, color=quality_level)) + 
  geom_point() +
   geom_smooth(method="lm", fill=NA)
printRegression(alcohol ~ chlorides)
#TODO
...
ggplot(data = wine, aes(x=chlorides, y=sulphates, color=quality_level)) + 
  geom_point() +
   geom_smooth(method="lm", fill=NA)
printRegression(sulphates ~ chlorides)
#TODOs
```
