...
getCI <- function(){
  calcSD(0.5, (total_pageviews_control + total_pageviews_exp))  
}


sanity_df <- as.data.frame(matrix(nrow = 2, ncol = 4))
colnames(sanity_df) <- c("metric", "lower bound", "upper bound", "observed", "passes")
sanity_df[1,] <- c("gross conversion", gross_p[[3]], 5, "yes")
sanity_df[2,] <- c("net conversion", net_p[[3]], 5, "no")
sanity_df
```
...
sign_df[1,] <- c("gross conversion", gross_p[[3]], 5, "yes")
sign_df[2,] <- c("net conversion", net_p[[3]], 5, "no")
sign_df
```
